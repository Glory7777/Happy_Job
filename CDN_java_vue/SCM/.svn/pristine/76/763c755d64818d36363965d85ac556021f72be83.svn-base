...
<script type="text/javascript">
			...
			switch (btnId) {
				...
				case 'btnClose' :
					...
					if($("#action").val() == "I") {
	    				...
	    				fn_searchlist($("#currentPageEmInfo").val());
    ...
    function fn_searchlist(currentPage) {
       ...
       var listcallback = function(returndata) {
    		...
    		$("#emInfoList").empty().append(returndata);
    		...
    		//swal(paginationHtml);
    		$("#emInfoPagination").empty().append( paginationHtml );
    		
    		// 현재 페이지 설정
    		$("#currentPageEmInfo").val(currentPage);
    		
    	}
    	
    	callAjax("/scm/emInfoList.do", "post", "text", true, param, listcallback);
	}
...
<body>
	<form id="myForm" action="" method="">
		<input type="hidden" id="currentPageEmInfo" /> <input type="hidden"
						...
						<div class="content">
							...
							<p class="Location">
								...
								<span class="btn_nav bold">시스템 관리</span> <span
									...
									href="/scm/emInfoList.do" class="btn_set refresh">새로고침</a>
							</p>

							<p class="conTitle">
								<span>직원정보 관리</span> <span class="fr"> <input
									type="checkbox" id="searchDel" name="searchDel" /> <label
									for="searchDel">삭제된 정보만 표시</label> <select id="searchUserType"
									name="searchUserType">
									...
									style="width: 200px; height: 28px;" placeholder="직원명을 입력해주세요" />
							...
							<div class="divComGrpCodList">
								<table class="col">
									...
									<tbody id="emInfoList"></tbody>
								</table>
							</div>

							<div class="paging_area" id="emInfoPagination"></div>
		...
		<!--// 모달팝업 -->
		<div id="emInfoPop" class="layerPop layerType2" style="width: 70%;">
			<dl>
				<dt>
					<strong>회원관리(내부직원)</strong>
				</dt>
				<dd class="content">
