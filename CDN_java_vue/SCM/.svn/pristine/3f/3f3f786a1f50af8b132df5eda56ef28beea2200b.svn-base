...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			selectComCombo("cli","clisel","all","","");  //  value : sup_cd 납품업체코드
			
			$('#clisel').change(function() {
				productCombo("c","cprosel","all",$("#clisel").val(),"");  //  value : pro_cd 상품코드   $("#clisel").val() 선택된 납품업체 코드
			});
			
			
			selectComCombo("pro","prosel","all","","");  //  value : pro_cd 상품코드
			
			selectComCombo("uc","ucsel","all","C","");  //  value : loginid 사용자ID   "C" 고객 타입
						
			productCombo("l","lsel","all","","");  //  value : ct_cd 카테고리코드
			
			$('#lsel').change(function() {
				productCombo("p","pprosel","all",$("#lsel").val(),"");  //  value : pro_cd 상품코드   $("#lsel").val() 선택된 카테고리 코드
			});
			
			$('#prosel').change(function() {
				selectComCombo("cli","cliprosel","all",$("#prosel").val(),"");  //  value : sup_cd 납품업체코드
			});
						  ...
						  if(image.files[0]){
                            ...
                            var fileSize = image.files[0].size;
							 ...
							 if(fileExt == "jpg" || fileExt == "png" || fileExt == "gif" || fileExt == "jpeg") {
								 this.inserthtml = "<img src='" + window.URL.createObjectURL(image.files[0]) + "'  style='width: 100px; height: 100px;' />";
							 } else {
								 this.inserthtml = "";
							 }
						  }
...
/////////////////////////sep scm 공지사항///////////////////////
	function fn_serachlist(currentPage) {
       ...
       var listcallback = function(returndata) {
    		...
    		dashNotice.datalist = returndata.listnotice;
    		dashNotice.totalcnt = returndata.totalcnt;
    		
    		var paginationHtml = getPaginationHtml(currentPage, dashNotice.totalcnt, pageSize, pageBlockSize, 'fn_serachlist');
    		console.log("paginationHtml : " + paginationHtml);
    		
    		dashNotice.noticePagination = paginationHtml;
    		
    		// 현재 페이지 설정
    		dashNotice.currentPage = currentPage;
    ...
    function fn_selectnoticedile(notice_cd) {
    	...
    	callAjax("/scm/vueNoticeselect.do", "post", "json", true, param, selectcallback);
   ...
   function fn_popupintfile(data) {
	   ...
	   noticepopfile.file_extention = data.file_extention;
	   
	   console.log(data.logic_path);
	   
	   noticepopfile.inserhtml = "<a href='javascript:fn_download()'>";
       	
       	if(data.file_nm == "" || data.file_nm == null) {
       		noticepopfile.inserhtml += "";
       	} else {
       		noticepopfile.inserhtml = "<a href='javascript:fn_download()'> "
       		
          	         if(noticepopfile.file_extention.toLowerCase() == "jpg" ||  noticepopfile.file_extention.toLowerCase() == "png"  ||  noticepopfile.file_extention.toLowerCase() == "gif" ||  noticepopfile.file_extention.toLowerCase() == "jpeg" ) {
          	        	noticepopfile.inserhtml += "<img src='" + data.logic_path + "'   style='width: 100px; height: 100px;''   />";
   		         } else {
   		        	noticepopfile.inserhtml += data.file_nm;
   		         }        		
       		    
       		noticepopfile.inserhtml += "</a>"; 
       	}
       	
       	noticepopfile.filepreviewDash = noticepopfile.inserhtml;
				...
				<li class="contents">
					...
					<div class="content" style="margin-bottom:20px;">
						...
						<c:choose>
								...
								<jsp:include
								page="/WEB-INF/view/dashboard/vueDashboardScm.jsp"></jsp:include>
	    					</c:when>
	    					
	    					<c:when test="${sessionScope.userType eq 'D'}">
								<jsp:include
								page="/WEB-INF/view/dashboard/vueDashboardDlm.jsp"></jsp:include>
	    					</c:when>
	    					
	    					<c:when test="${sessionScope.userType eq 'C'}">
								<jsp:include
								page="/WEB-INF/view/dashboard/vueDashboardCus.jsp"></jsp:include>
	    					</c:when>
	    					
	    					<c:when test="${sessionScope.userType eq 'P'}">
								<jsp:include
								page="/WEB-INF/view/dashboard/vueDashboardPcm.jsp"></jsp:include>
	    					</c:when>
	    					
	    					<c:when test="${sessionScope.userType eq 'E'}">
								<jsp:include
								page="/WEB-INF/view/dashboard/dashboardExe.jsp"></jsp:include>
	    					</c:when>
						</c:choose>
						</br>
						<p class="conTitle">
						...
						<div id="dashNotice" class="dashNotice">
							<table class="col">
								...
								<tbody>
									...
									<template v-else v-for="one in datalist" >
										<tr @click="fn_selectnoticedile(one.notice_cd)">
											<td>{{ one.notice_cd }}</td>
											<td>{{ one.notice_title }}</td>
				...
				<table class="row">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3">
							      <input type="text" class="inputTxt p100" name="notice_titlefile" id="notice_titlefile" v-model="notice_titlefile" readonly/>
							 </td>
						</tr>
						<tr>
							<th scope="row">공지내용 <span class="font_red">*</span></th>
							<td colspan="3">
							     <textarea name="notice_contentfile" id="notice_contentfile" v-text="notice_contentfile" rows="5" cols="30" readonly ></textarea>
						    </td>
						</tr>
						<tr>
							...
							<td colspan="2">
							      <div id="filepreviewDash" v-html="filepreviewDash"></div>
							 </td>
						</tr>						
						    ...
						    <table>
						        ...
						        <tbody>
						             <tr> 
						                  ...
						                  <td> 
						                       <select id="clisel" name="clisel"> </select>
						                  </td>
						                  <td> 
						                       공급업체별 제품
						                  </td>
						                  <td> 
						                      <select id="cprosel" name="cprosel"> </select>	
						                  </td>		
						                  <td> 
						                      제품
						                  </td>						                      					                  
						                  <td> 
						                    <select id="prosel" name="prosel"> </select>	
						                  </td>
						                  <td> 
						                     사용자   
						                  </td>
						                  <td> 
						                       <select id="ucsel" name="ucsel"> </select>	
						                  </td>
						             </tr>
                                     <tr> 
						                  <td> 
						                  ...
						                  <td> 
						                       <select id="lsel" name="lsel"> </select>
						                  </td>
						                  <td> 
						                       카테고리별 제품
						                  </td>
						                  <td> 
						                      <select id="pprosel" name="pprosel"> </select>	
						                  </td>		
						                  <td> 
						                       제품별 공급업체
						                  </td>						                      					                  
						                  <td> 
						                     <select id="cliprosel" name="cliprosel"> </select>	
						                  </td>
						                  <td> 
