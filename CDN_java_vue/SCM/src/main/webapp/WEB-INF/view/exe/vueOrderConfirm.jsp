   ...
   $(function() {
		...
		$('a[name=btn]').click(function(e) {
			...
			switch (btnId) {
				...
				case 'btnClose' :
					...
					if(approvalPop.action == "U") {
	    				fn_approvalList(approvalList.currentPage);
	    			}					
					break;					
			}
		});
	...
	function fn_approvalList(currentPage) {
		...
		var param = {
    			...
    			searchAppr : searchArea.searchAppr,
    			searchStartDate : searchArea.searchStartDate,
    			searchEndDate : searchArea.searchEndDate
		}
		
		console.log("##########searchAppr : "+searchArea.searchAppr);
		
		var listCallBack = function(returnData) {
			
			console.log(JSON.stringify(returnData));
			
			approvalList.dataList = returnData.approvalList;
			approvalList.totalCnt = returnData.totalCnt;	
			
			var paginationHtml = getPaginationHtml(currentPage, approvalList.totalCnt, pageSize, pageBlockSize, 'fn_approvalList');
			
			console.log("paginationHtml : "+paginationHtml);
			
			approvalList.pagingNavi = paginationHtml;
    		
			approvalList.currentPage = currentPage;
			
		}	
		
		callAjax("/exe/approvalListvue.do", "post", "json", true, param, listCallBack);
	...
	function fn_selectPurchase(pur_cd) {
	 ...
	 function fn_popupInt(data) {	 
		 	...
		 	approvalPop.pur_cd = data.pur_cd;
		 	approvalPop.pro_cd = data.pro_cd;
		 	approvalPop.pro_nm = data.pro_nm;
		 	approvalPop.pro_sup_price = data.pro_sup_price;
		 	approvalPop.pro_unit_price = data.pro_unit_price;
		 	approvalPop.pro_stock = data.pro_stock;
		 	approvalPop.pur_cnt = data.pur_cnt;
		 	approvalPop.pur_total_unit_price = data.pur_total_unit_price;
		 	approvalPop.sup_cd = data.sup_cd;
		 	approvalPop.sup_nm = data.sup_nm;
		 	approvalPop.pur_nm = data.pur_nm;
		 	approvalPop.pur_date = data.pur_date;
		 	approvalPop.pur_appr_date = data.pur_appr_date;
		 	approvalPop.action = "U"
							
						
			if(data.pur_appr == '0') {
				
				approvalPop.pur_appr ='미승인';
				
			} else if(data.pur_appr == '1') {
				
				approvalPop.pur_appr ='승인';
				
			} else if(data.pur_appr == '2') {
				
				approvalPop.pur_appr ='입금 완료';
				
			} else if(data.pur_appr == '3') {
				
				approvalPop.pur_appr ='입고 완료';
			...
			if(data.admin_nm == null) {
				...
				approvalPop.admin_nm = "";
				
				$("#btnSave").show();
				
			} else {
				// admin_nm (승인자 이름) vale가 null이 아닌 경우 승인 버튼(btnSave)을 숨김
				approvalPop.admin_nm = data.admin_nm;
	...
	function fn_Save() {    	
    	...
    	var param = {
    			action : approvalPop.action,
    			pur_cd : approvalPop.pur_cd,
                       ...
                       <div id="searchArea">
						<p class="conTitle">  
							...
							<span class="fr">
							    ...
							    <a class="btnType blue" @click="fn_approvalList();" name="modal"><span>검색</span></a>							    
							</span> 							
						</p>
						<br>
						
						<label style="display: flex; justify-content: flex-end;">
							<input type="checkbox" name="searchAppr" id="searchAppr" v-model="searchAppr" @change="noAppr()">&nbsp;<span>미승인 목록 조회</span>
						</label>	
						</div>					
						
						<div id="approvalList" class="approvalList">
							<table class="col">
				...
				<table class="row">
					...
					<tbody>
						<tr>							
							...
							<td id="pro_sup_price" v-text="pro_sup_price">
							 </td>
						...
						<tr>							
							 <th scope="row">재고</th>
							<td id="pro_stock" v-text="pro_stock">
							 </td>
