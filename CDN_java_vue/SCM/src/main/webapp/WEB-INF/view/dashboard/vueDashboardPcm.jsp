...
<script src='${CTX_PATH}/js/sweetalert/sweetalert.min.js'></script>
    ...
    var calendar;
        ...
        calendar = new FullCalendar.Calendar(calendarEl, {
		  ...
		  dateClick: function(info) {
			...
			// 선택된 날짜의 이벤트 목록 가져오기
			  ...
			  $.ajax({
			    url: "/dashboard/listPurchaseDirectionDashboard.do",
			    type: "POST",
			    data: {
			    		thisDate : fullDate
			    },
			    ...
			    success: function (res) {
			      if (res.length) {
			        ...
			        for (var i =0; i<res.length;i++){
			        		text+= (i+1)+'. '+ res[i].pur_nm+'의 '+res[i].pro_nm +'\n'
			        		text+= '[진행상황] : ' + res[i].pur_appr+'\n\n'
			        }
			        swal(text);
			      } else {
			        // 이벤트 목록이 없을 때
     ...
     // 일정 조회
  	...
  	function fCalendar() {
  		...
  		callAjax("/dashboard/listPurchaseDirectionDashboard.do", "post", "json", true, param, resultCallback);
  	}
	...
	function fRedrawCalendar(data) {
		for (var i=0;i<data.length;i++){
			calendar.addEvent( {
				'title':''+data[i].pur_nm+'의 '+data[i].pro_nm+'', 
				'start':''+data[i].pur_date+'', 
				});
		}
	}
